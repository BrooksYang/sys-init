      ...
      <div class="alert alert-info">
    	...
    	<div class="row">
	    	<div class="col-md-12">
              ...
              <input class="form-control input-lg required" type="text" name="name"  id="name" placeholder="请输入用户名" value="{{ $res->name }}" disabled>
              <div id="name_validate"></div>
              </div>              
              <div class="form-group">
              <input class="form-control input-lg required email" type="text" name="email"  id="email" placeholder="请输入邮箱地址" value="{{ $res->email }}">
              <div id="email_validate"></div>
              </div>  

		          <div class="form-group">
		              <button type="button" class="btn btn-default pull-right" onclick="updateSupervisor()">提交</button>
		          </div>
				</form>
		    </div>
		</div>
...
<script type="text/javascript">
  ...
  function updateSupervisor(){
      ...
      $.ajax({
          type : 'POST',
          url : "{{ url('ticket/supervisor/update').'/'.$res->id }}",
          dataType : 'json',
