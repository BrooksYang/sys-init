...
<?php $model = 'bannerWap'; $logoName = config("imgCrop.$model.name"); ?>
@section('css-part')
    {{-- 图片裁剪 --}}
            ...
            <div class="box">
                <div class="box-header">
                    ...
                    <h3 class="box-title">
                        ...
                        <span>{{ @$editFlag ? '编辑首页轮播图-Wap端' : '添加首页轮播图-Wap端' }}</span>
                    </h3>
                </div>

                {{-- Form --}}
                <div class="box-body">
                    <form class="form form-horizontal" id="form" role="form" method="POST" action="{{ @$editFlag ? url("portal/ads/wap/$bannerWap->id") : url('portal/ads/wap') }}">
                        {{ csrf_field() }}
                        ...
                        <div class="form-group {{ $errors->has($logoName) ? 'has-error' : '' }}">
                            <div class="col-md-8">
                                <label>上传轮播图-Wap端 <span style="color: #666;font-weight:normal;">({{ config("imgCrop.$model.crop.width") }}*{{ config("imgCrop.$model.crop.height") }})</span></label>
    ...
    <script>
        $(function () {
            ...
            $('#form').submit(function () {
                if (!hasUpload && !$('#thumbnail').val()) { layer.msg('请上传封面图'); return false;}
